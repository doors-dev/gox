...
- when custom printers or proxies inspect `JobOpen.Attrs`
...
A common implementation pattern is a proxy printer: call `elem.Print(cur.Context(), customPrinter)`, inspect the first `*gox.JobOpen`, adjust it, then forward the rest into the current cursor.
